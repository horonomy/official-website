...
// Session inclusion is strictly below 1000ms/5000ms, starting at the first contributing shift.
...
test('CLS preserves early-start clusters and splits at one-second and five-second boundaries',()=>{
  ...
  const afterZero=[shift(200,0),...[600,1400,2200,3000,3800,4600,5400].map(start=>shift(start))];
  assert.equal(cumulativeLayoutShift(afterZero),.875);
